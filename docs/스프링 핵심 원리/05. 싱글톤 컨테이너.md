...
sequenceDiagram
    ...
    participant G as AppConfig CGLIB
    participant R as memberRepository 빈

    C->>G: memberService() 호출
    G->>R: memberRepository 최초 생성·등록
    C->>G: orderService() 호출
    G->>C: memberRepository 등록 여부 확인
    C-->>G: 기존 R 반환
    G-->>C: 같은 R을 orderService에 주입
```

겉으로는 `memberRepository()`를 여러 번 호출하지만, 향상된 설정 클래스가 컨테이너를 확인해 이미 등록된 빈을 돌려주므로 모든 서비스가 같은 저장소 인스턴스를 공유한다.
