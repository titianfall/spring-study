...
```mermaid
sequenceDiagram
    participant C as OrderApp 또는 테스트
    participant M as MemberService
    participant O as OrderService
    participant R as MemberRepository
    participant D as DiscountPolicy

    C->>M: join(VIP 회원)
    M->>R: save(member)
    C->>O: createOrder(memberId, item, price)
    O->>R: findById(memberId)
    R-->>O: VIP 회원
    O->>D: discount(member, price)
    D-->>O: 할인 금액
    O-->>C: Order 결과
```

실행 코드와 테스트는 진입점만 다를 뿐 같은 협력 흐름을 검증한다. 주문 서비스는 회원 저장소와 할인 정책에 각 책임을 위임하고, 계산 결과를 `Order` 객체로 조립해 반환한다.
