...
flowchart TD
    request["브라우저 요청"]
    mapping{"컨트롤러 매핑이 있는가?"}
    body{"@ResponseBody인가?"}
    static["static 파일 탐색"]
    view["viewResolver + Thymeleaf"]
    converter["HttpMessageConverter"]
    html["HTML 응답"]
    api["문자열 또는 JSON 응답"]

    request --> mapping
    mapping -- "없음" --> static --> html
    mapping -- "있음" --> body
    body -- "아니오" --> view --> html
    body -- "예" --> converter --> api
```

세 방식은 요청을 받는 입구는 같지만, **컨트롤러 매핑 여부**와 **`@ResponseBody` 사용 여부**에서 처리 경로가 갈라진다.
...
sequenceDiagram
    ...
    participant R as Static Resource

    B->>S: GET /hello-static.html
    S->>C: 매핑 조회
    C-->>S: 매핑 없음
    S->>R: static/hello-static.html 탐색
    R-->>B: 파일 내용을 그대로 응답
```
...
```mermaid
flowchart LR
    request["브라우저 요청"] --> jsp["JSP 한 파일"]
    jsp --> logic["DB 조회와 비즈니스 로직"]
    jsp --> render["HTML 생성"]
    render --> response["브라우저 응답"]
```
...
sequenceDiagram
    ...
    B->>C: GET /hello-mvc?name=spring
    C->>M: name=spring 저장
    C-->>V: 뷰 이름 hello-template
    M-->>V: 렌더링 데이터 전달
    V-->>B: 완성된 HTML 응답
```
