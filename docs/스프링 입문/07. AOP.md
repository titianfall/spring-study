...
flowchart LR
    request["메서드 호출"]
    proxy["AOP 프록시<br/>START와 시간 측정 시작"]
    target["핵심 로직<br/>join 또는 findMembers"]
    finish["AOP 프록시<br/>END와 소요 시간 출력"]
    result["호출자에게 결과 반환"]

    request --> proxy --> target --> finish --> result
```

호출자는 실제 `MemberService`를 직접 호출하는 것처럼 보이지만, 스프링이 만든 프록시가 앞에서 호출을 가로채 공통 로직을 실행한 뒤 실제 대상을 호출한다.
