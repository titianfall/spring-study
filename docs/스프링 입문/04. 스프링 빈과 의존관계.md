...
```mermaid
flowchart TD
    scan["@SpringBootApplication<br/>컴포넌트 스캔 시작"]
    controller["@Controller<br/>MemberController"]
    service["@Service<br/>MemberService"]
    repository["@Repository<br/>MemoryMemberRepository"]
    container["스프링 컨테이너"]
    ...
    container -->|"생성자 주입"| controller
    container -->|"생성자 주입"| service
```

컴포넌트 스캔은 클래스를 **발견하고 빈으로 등록**하는 단계이고, `@Autowired`는 등록된 빈 사이의 **의존관계를 연결**하는 단계다.
...
flowchart LR
    container["스프링 컨테이너"]
    service["memberService<br/>단일 인스턴스"]
    repository["memberRepository<br/>단일 인스턴스"]
